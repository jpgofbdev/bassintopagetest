...
document.getElementById('topagecebbox-checkbox').addEventListener('change', function(event) {
    const baseURL3 = 'https://services.sandre.eaufrance.fr/geo/sandre';
    ...
    console.log('URL généréeeee:', urlbb3);
    ...
    .then(data => {
        ...
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(data, "text/xml");
        const features = xmlDoc.getElementsByTagName('sa:CoursEau_FXX_Topage2024');

        // Réinitialiser les lignes
        ceLayer.clearLayers();

        // Extraire les données et créer des polylines
        Array.from(features).forEach(feature => {
            const msGeometry = feature.getElementsByTagName('sa:msGeometry')[0];
            const lineString = msGeometry.getElementsByTagName('gml:LineString')[0];
            const posList = lineString.getElementsByTagName('gml:posList')[0]?.textContent.trim().split(' ');

            if (posList) {
                console.log('posList:', posList);  // Log des coordonnées brutes

                const latLonPairs = [];
                for (let i = 0; i < posList.length; i += 2) {
                    // [lat, lon] -> vérifier si l'ordre est correct
                    const lat = parseFloat(posList[i]);
                    const lon = parseFloat(posList[i+1]);
                    console.log(`Lat: ${lat}, Lon: ${lon}`);  // Log des paires de coordonnées
                    latLonPairs.push([lat, lon]);
                }

                console.log('latLonPairs:', latLonPairs);  // Log des paires de coordonnées formatées

                // Créer et ajouter la polyline à la carte
                const polyline = L.polyline(latLonPairs, { color: 'blue' });
                polyline.bindPopup('Informations sur le cours d\'eau');
                polyline.addTo(ceLayer);  // Ajouter à la couche existante
            } else {
                console.warn('Aucun posList trouvé dans la réponse WFS.');
            }
        });

        // Afficher ou retirer la couche de la carte selon l'état de la case à cocher
        if (event.target.checked) {
            ceLayer.addTo(map);  // Afficher la couche
        } else {
            ceLayer.clearLayers();  // Effacer la couche
        }
